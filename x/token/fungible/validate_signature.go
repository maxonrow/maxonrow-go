...
func (k Keeper) ValidateSignatures(ctx sdkTypes.Context, msg sdkTypes.Msg) sdkTypes.Error {
	...
	case MsgSetFungibleTokenAccountStatus:
		fromSignature = NewSignature(msg.TokenAccountPayload.PubKey, msg.TokenAccountPayload.Signature)
		// * from sign bytes
		fromSignBytes = msg.TokenAccountPayload.TokenAccount.GetAccountStatusSettingFromSignBytes()
		fromAddr = msg.TokenAccountPayload.TokenAccount.From
		fromAccountNonce = msg.TokenAccountPayload.TokenAccount.Nonce

		//* issuer sign bytes
		issuerSignBytes = msg.TokenAccountPayload.GetAccountStatusSettingSignBytes()
		issuerSignatures = msg.Signatures
	default:
		errMsg := fmt.Sprintf("Invalid signature: %v", msg.Type())
